...
- [~] **Ops:** rate limiting, Sentry, E2E (Playwright) for extract+sync.
  ...
  - [x] DONE 2026-09-12 (`72e0588`): 7-day npm release-age cooldown — `.npmrc` (`min-release-age=7`, requires npm >= 11.10; lockfile-pinned versions unaffected, `--min-release-age=0` bypass documented for vetted security patches) + Dependabot `cooldown: default-days: 7` on the npm ecosystem + CI upgrades npm to v11 in both jobs (bundled npm 10.x on Node 20/22 silently ignores the key); regression test `tests/supply-chain.test.ts`.
