...
- [ ] Listen reliability (plan: `docs/listen-reliability.md`) — incident was a dead server, not a TTS defect; make listen failures audible (player status + error surfacing + reader error boundary) and prove click→speak with mocked speechSynthesis tests
- [ ] Archive (plan: `docs/archive.md`) — `archived` flag + `archivedAt`, Archived view with unarchive, `GET /api/articles` defaults to active-only, archived excluded from Kindle
- [ ] Send to Kindle (plan: `docs/kindle.md`) — one-click email of latest 50 active articles as one compiled HTML doc via user SMTP (`nodemailer`), status endpoint + setup docs
