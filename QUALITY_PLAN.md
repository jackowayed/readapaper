...
- [x] 2026-09-12: 7-day npm release-age cooldown — `.npmrc` `min-release-age=7` (npm >= 11.10; lockfile pins unaffected) + Dependabot npm `cooldown: default-days: 7` + CI npm 11 upgrade in both jobs + `tests/supply-chain.test.ts` (2 tests). Verified: `npm run typecheck`, `npm run lint`, `npm run format:check`, `npm test` (109/109).
